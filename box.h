 ...
 //     assert(min < max);
	...
	//
	 bool overlap(const Box &box) {
		 return (min().x() <= box.parameters[1].x() && max().x() >= box.parameters[0].x()) &&
         (min().y() <= box.parameters[1].y() && max().y() >= box.parameters[0].y()) &&
         (min().z() <= box.parameters[1].z() && max().z() >= box.parameters[0].z());
	}
