...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
    ...
    for (int i = 0; i < boxList.size(); i++){
        count = getMeshPointsInBox(mesh, node.points, boxList[i], pointsRtn);
        if(count > 0){
            TreeNode newNode;
            newNode.box = boxList[i];
            newNode.points = pointsRtn;
   ...
   if(node.children.size() > 0) node.points.clear();
}
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
    ...
    if(node.box.intersect(ray, -INFINITY, INFINITY)) {
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn, vector<int> & pointsList) {
	bool intersects = false;
    if (node.box.overlap(box)){
        if (node.children.size() == 0){
            boxListRtn.push_back(node.box);
            pointsList.push_back(node.points.at(0));
            return true;
        } else {
            for(int i = 0; i < node.children.size(); i++){
                intersects = intersect(box, node.children[i], boxListRtn, pointsList);
            }
            if (intersects) return intersects;
        }
    }
